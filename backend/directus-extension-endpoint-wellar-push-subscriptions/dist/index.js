...
function fail(stage, status, code) {
...
function asSafeString(value, maxLength) {
...
async function resolveWorkspaceContext(context, userId) {
...
async function upsertSubscription(trx, { userId, businessProfileId, payload, }) {
    ...
    return {
        operation,
        duplicateRowsDeactivated: typeof duplicateRowsDeactivated === 'number'
            ? duplicateRowsDeactivated
            : Number(duplicateRowsDeactivated ?? 0) || 0,
    };
}
async function revokeSubscription(trx, { userId, businessProfileId, deviceId, }) {
    const now = new Date().toISOString();
...
function mapUnexpectedDatabaseFailure(error) {
...
function logUnexpectedFailure(context, stage, error) {
    context?.logger?.error?.({
        event: 'wellar_push_sync_failed',
        stage,
        error_class: errorClass(error),
        error_code: error instanceof EndpointError
            ? error.code
...
function handleEndpointError(context, res, error, fallbackStage, fallbackCode) {
    ...
    handler: (router, context) => {
        router.post('/sync', async (req, res) => {
            ...
            try {
                ...
                return handleEndpointError(context, res, error, 'upsert', 'PUSH_SYNC_FAILED');
            }
        });
        router.post('/revoke', async (req, res) => {
            ...
            try {
                ...
                return handleEndpointError(context, res, error, 'upsert', 'PUSH_REVOKE_FAILED');
            }
        });
    },
};
