...
function membershipRoleRank(role) {
    ...
    if (normalized === 'manager' || normalized === 'manger')
        return 2;
    if (normalized === 'employee' || normalized === 'member' || normalized === 'user')
        return 1;
    return 0;
}
...
async function getOrCreateDirectusUser({ services, schema, database, email, firstName, lastName, allowCreate, defaultRoleId, }) {
    ...
    const createdId = (await usersService.createOne(createPayload));
    const created = (await usersService.readOne(createdId, {
        fields: ['id', 'email', 'first_name', 'last_name', 'role'],
    }));
    return created;
}
async function createDirectusSessionForUser({ services, schema, database, user, req, }) {
        ...
        async () => {
            if (typeof authService.createTokenPair === 'function') {
                ...
                return {
                    ...
                    expires: Number(pair.expires ?? 900000),
                };
            }
            return null;
        },
...
export default defineEndpoint((router, context) => {
    ...
    router.post('/member-identities', async (req, res) => {
        ...
        try {
            ...
            const callerMembership = callerMemberships
                .slice()
                .sort((a, b) => membershipRoleRank(String(b.member_role ?? '')) - membershipRoleRank(String(a.member_role ?? '')))[0];
