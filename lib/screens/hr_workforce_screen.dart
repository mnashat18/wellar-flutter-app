...
class HrWorkforceScreen extends ConsumerStatefulWidget {
  const HrWorkforceScreen({super.key});

  @override
  ConsumerState<HrWorkforceScreen> createState() => _HrWorkforceScreenState();
}

class _HrWorkforceScreenState extends ConsumerState<HrWorkforceScreen> {
  static const String _allDepartmentsValue = '';
  static const String _unassignedDepartmentValue = '__unassigned__';

  late Future<HrWorkforceDirectoryData> _future;
  ...
  void initState() {
    ...
    _future = _loadWorkforceDirectory(forceRefresh: false);
    _workspaceSubscription = ref.listenManual(activeWorkspaceContextProvider, (
      previous,
      next,
    ) {
      ...
      if (nextRole != 'hr') {
        debugPrint(
          '[STALE_ROLE_WIDGET_BLOCKED] widget=HrWorkforceScreen expected=hr actual=$nextRole',
        );
        setState(() {
      ...
      setState(() {
        ...
        _departmentFilter = _allDepartmentsValue;
        _query = '';
        _future = _loadWorkforceDirectory(forceRefresh: true);
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (activeRole != 'hr') {
      debugPrint(
        '[STALE_ROLE_WIDGET_BLOCKED] widget=HrWorkforceScreen expected=hr actual=$activeRole',
      );
      return Scaffold(
        backgroundColor: Colors.transparent,
        body: Center(
          child: WellarErrorState(
            title: Tr.t(lang, 'access_unavailable'),
            body: Tr.t(lang, 'role_unavailable_info'),
            onRetry: () => setState(
              () => _future = _loadWorkforceDirectory(forceRefresh: true),
            ),
          ),
    ...
    return Scaffold(
      ...
      body: AnimatedWellarScreen(
        ...
        child: FutureBuilder<HrWorkforceDirectoryData>(
          future: _future,
          builder: (context, snapshot) {
            ...
            if (snapshot.hasError) {
              ...
              return SmartEmptyState(
                ...
                onAction: () => setState(
                  () => _future = HrOpsService.instance.fetchWorkforceDirectory(
                    forceRefresh: true,
                  ),
                ),
              );
            ...
            final businessProfileId = (workspace?.businessProfileId ?? '')
                .trim();
            final membershipId = (workspace?.membershipId ?? '').trim();
            final role =
                (workspace?.finalEffectiveRole ??
                        workspace?.memberRole ??
                        'unknown')
                    .trim();
            final departmentId = (workspace?.departmentId ?? '').trim();
            debugPrint(
              '[WORKFORCE_SOURCE] screen=HrWorkforce role=$role business_profile=$businessProfileId membership_id=$membershipId department=${departmentId.isEmpty ? "none" : departmentId}',
            );
            debugPrint(
              '[HR_WORKFORCE_RENDER] loaded_count=${data.members.length} visible_count=${filtered.length}',
            );
            if (_query.isNotEmpty) {
              debugPrint(
                '[WORKFORCE_FILTER] query=$_query visible_count=${filtered.length}',
              );
            }
  ...
  List<HrMemberView> _applyFilter(List<HrMemberView> data) {
  ...
  Color _memberAccent(HrMemberView item) {
  ...
  Future<HrWorkforceDirectoryData> _idleWorkforceFuture() {
    return Future<HrWorkforceDirectoryData>.value(
      const HrWorkforceDirectoryData(members: [], departments: []),
    );
  }

  Future<HrWorkforceDirectoryData> _loadWorkforceDirectory({
    required bool forceRefresh,
  }) {
    return HrOpsService.instance
        .fetchWorkforceDirectory(forceRefresh: forceRefresh)
        .timeout(
          const Duration(seconds: 20),
          onTimeout: () {
            throw TimeoutException('HR workforce request timed out.');
          },
        );
  }
}
